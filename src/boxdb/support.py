...
from boxdb.settings import PRIMARY_KEY,NOT_NULL,UNIQUE
...
def get_not_null_columns(table_name):
    """
    This gets all the not null rows from the table
    """
    return get_content("row",NOT_NULL(table_name))

def get_unique_columns(table_name):
    """
    This gets all the uniquee rows from the table
    """
    print(UNIQUE(table_name))
    return get_content("row",UNIQUE(table_name))
